...
def generate_badge_definition(name: str, config: BadgeConfig, base64_content: str = None) -> str:
    """Generate essential badge definition"""
    badge_name = name.lower()
    
    # Standard logo or base64 SVG
    logo = f"data:image/svg+xml;base64,{base64_content}" if base64_content else badge_name
    ...
    output.append(f"[{badge_name}-badge]: https://img.shields.io/badge/{config['name']}-{color}?{query_string}")
